...
def tune_joint(
    ...
    best_cfg = None
    best_ndcg = -1.0
    for eta in [0.24, 0.26, 0.28, 0.30]:
        for alpha in [0.28, 0.30, 0.32]:
            for domain_boost in [0.07, 0.08]:
                for venue_boost in [0.003, 0.005, 0.008]:
                    for year_boost in [0.00, 0.01]:
                        for sparse_beta in [0.75, 0.80, 0.85]:
...
def tune_stage2_rrf(
    ...
    best_ndcg = -1.0
    for gamma_rrf in [0.0, 0.005, 0.01, 0.015, 0.02]:
...
def main() -> None:
    ...
    # v9 fast search: sparse_beta is fixed to 1.0, so no TA-only sparse branch needed.
    public_sparse_ta_scores = public_sparse_scores
    heldout_sparse_ta_scores = heldout_sparse_scores
    ...
    rrf_hold = rrf_from_full_scores(heldout_minilm_scores, sparse_mix_hold, RRF_K)
